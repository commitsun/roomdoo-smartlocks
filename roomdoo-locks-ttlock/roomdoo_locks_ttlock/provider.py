...
class TTLockProvider(BaseLockProvider):
    ...
    def _handle_response(self, response: requests.Response) -> dict:
        ...
        if not response.ok:
            raise LockConnectionError(f"HTTP error [{response.status_code}]: {response.text}")
        try:
            body = response.json()
        except requests.exceptions.JSONDecodeError as err:
            raise LockAPIError("Invalid response from TTLock API") from err
    ...
    def _to_ms(self, value: datetime | int) -> int:
    ...
    def _post(self, path: str, payload: dict) -> dict:
        try:
            ...
            raise LockConnectionError(f"Failed to connect to TTLock API: {e!s}") from e
    ...
    def _change_passcode(self, lock_id: str, code_id: str, starts_at: datetime, ends_at: datetime) -> None:
        self._post(
    ...
    def _delete_passcode(self, lock_id: str, code_id: str) -> None:
        self._post(
    ...
    def _read_pin(self, lock_id: str, code_id: str) -> str:
        ...
        return next(
            (p["keyboardPwd"] for p in data.get("list", []) if str(p["keyboardPwdId"]) == str(code_id)),
            "",
        )
    ...
    def get_lock_info(self, lock_id: int) -> dict:
        return self._get(
            "/v3/lock/detail",
            {
                "clientId": self.clientId,
    ...
    def set_auto_lock_time(self, lock_id: int, seconds: int, type: int = 2) -> None:
        self._post(

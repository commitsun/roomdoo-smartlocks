...
class TestConnection(BaseProviderTest):
    ...
    def test_connection_auth_error(self):
        self.mock_svc.findAllRooms.return_value = error_result(
            "ERROR_SERVICE_AUTHENTICATION", "500"
        )
    ...
    def test_connection_license_error(self):
        self.mock_svc.findAllRooms.return_value = error_result(
            "ERROR_NOT_AUTHORIZED_IN_SERVER_SITE_LICENSE", "604"
        )
...
class TestRefRoundtrip(unittest.TestCase):
    def test_pack_unpack_roundtrip(self):
        ...
        self.assertEqual(
            TesaSmartairProvider._unpack_ref(TesaSmartairProvider._pack_ref(ref)), ref
        )
...
class TestGrantCheckin(BaseProviderTest):
    ...
    def test_grant_multiple_rooms_share_one_pin(self):
        ...
        keypads = [
            c.kwargs["guestData"]["keyPad"] for c in self.mock_svc.checkin.call_args_list
        ]
        self.assertEqual(keypads, ["123456", "123456"])

    def test_grant_occupied_room_raises_operation_error(self):
        self.mock_svc.checkin.return_value = error_result(
            "RESULT_ERROR_CHECKIN_ROOM_OCCUPIED", "300"
        )
        starts, ends = past_window()
...
class TestPinCollision(BaseProviderTest):
    ...
    def test_autogenerated_pin_gives_up_after_max_attempts(self):
        ...
        self.assertEqual(
            self.mock_svc.checkin.call_count, self.provider._MAX_PIN_ATTEMPTS
        )
...
class TestRevokeAccess(BaseProviderTest):
    ...
    def test_revoke_precheckin_cancels_pre_assignation(self):
        ...
        self.assertEqual(
            self.mock_svc.precheckinCancel.call_args.kwargs["preAssignationId"], 299
        )

    def test_revoke_is_idempotent_when_room_not_occupied(self):
        self.mock_svc.checkout.return_value = error_result(
            "RESULT_ERROR_CHECKIN_ROOM_NOT_OCCUPIED", "310"
        )
        ref = self._ref(False, [{"lock_id": "81", "code_id": "81"}])
        self.assertTrue(self.provider.revoke_access(ref))

    def test_revoke_is_idempotent_when_pre_assignation_invalid(self):
        self.mock_svc.precheckinCancel.return_value = error_result(
            "RESULT_ERROR_CHECKIN_INVALID_ROOM", "307"
        )
...
class TestExtras(BaseProviderTest):
    ...
    def test_add_pin_user_ok(self):
        fake_user = SimpleNamespace(
            userName="cleaning1", userCarrier="CAR_PIN", keyPad="3344"
        )
        self.mock_svc.userAdd.return_value = ok_result(userData=fake_user)
        result = self.provider.add_pin_user(
            "cleaning1", "3344", datetime(2026, 6, 1), datetime(2026, 12, 31)
        )
    ...
    def test_add_pin_user_collision_raises(self):
        self.mock_svc.userAdd.return_value = error_result(
            "ERROR_DATA_ERROR", "800", "PIN_ALREADY_EXISTS_PIN_USER"
        )
        with self.assertRaises(LockPinCollisionError):
            self.provider.add_pin_user(
                "cleaning1", "3344", datetime(2026, 6, 1), datetime(2026, 12, 31)
            )
    ...
    def test_open_door_unknown_raises_not_found(self):
        self.mock_svc.doorOpen.return_value = error_result(
            "ERROR_OPERATION_DOOR_UNKNOWN", "503"
        )
    ...
    def test_open_door_offline_raises(self):
        self.mock_svc.doorOpen.return_value = error_result(
            "ERROR_COMMUNICATION_NO_ANSWER", "C5"
        )
